...
if __name__ == "__main__":
    ...
    x_train = np.zeros(x_train_org.shape, dtype=np.uint8)
    x_test = np.zeros(x_test_org.shape, dtype=np.uint8)
    ...
    with LZMAFile("./FMNIST/graphs_train_adap.tar.lz", "wb") as f:
        pickle.dump({"graph": graphs_train, "y": y_train}, f)

    with LZMAFile("./FMNIST/graphs_test_adap.tar.lz", "wb") as f:
        pickle.dump({"graph": graphs_test, "y": y_test}, f)

    print("Graphs saved to ./FMNIST/graphs_train_adap.tar.lz and ./FMNIST/graphs_test_adap.tar.lz")
