...
from FMNIST_graph_gen_adap import generate_graphs
...
def create_graph_subset():
    with LZMAFile("./FMNIST/graphs_train_adap.tar.lz", "rb") as f:
        d = pickle.load(f)
    graphs_train: Graphs = d["graph"]

    (_, _), (x_test_org, y_test) = fashion_mnist.load_data()

    x_test = np.zeros(x_test_org.shape)
    for i in range(x_test.shape[0]):
        x_test[i] = cv2.adaptiveThreshold(x_test_org[i], 1, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
    ...
    return graphs, X.astype(float), Y, class_names




def plot_clauses(tm: MultiClassGraphTsetlinMachine, graphs: Graphs, X, Y, class_names):
...
if __name__ == "__main__":
    ...
    with LZMAFile("./FMNIST/fmnist_model_adap.tm", "rb") as f:
    ...
    fig, axd = plot_clauses(tm, graphs, X, Y, class_names)
    fig.savefig("./FMNIST/FMNIST_clauses_adap.png", dpi=300, bbox_inches="tight")
